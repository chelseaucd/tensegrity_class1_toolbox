...
% TENSEGRITY_STATICS Static equilibrium analysis of Class 1 tensegrity structures.
%
% This function performs static analysis for tensegrity systems, with specific 
% applicability to CLASS 1 STRUCTURES (where compression members/bars are 
% isolated from each other within a continuous network of tension members/strings).
%
% The function assumes the tension in all members is measurable and follows 
% this logic:
% 1. Solves the static equilibrium equations (A*x = u).
% 2. If the system is Statically Determinate:
%    - Solves directly for member forces.
% 3. If the system is Under-Determined (Redundant/Hyperstatic):
%    a) Checks "Pretensionability": Can the system maintain stability with 
%       tension >= tau_min in all strings under zero external load?
%    b) Optimizes Tension Distribution:
%       - If pretensionable: Finds a distribution that satisfies tau >= tau_min
%         while minimizing the L1 norm of tensions (w'*x_tilde).
%       - If NOT pretensionable: Finds a distribution that maximizes the 
%         minimum tension (max-min strategy) to attempt stabilization.
% 4. Computes reaction forces (V) at fixed nodes.
%
% INPUTS:
%   b   = Number of bars (compression members)
%   s   = Number of strings (tension members)
%   q   = Number of free nodes
%   p   = Number of fixed nodes
%   dim = Dimension of system (2 or 3)
%   Q   = (dim x q) Matrix of free node coordinates
%   P   = (dim x p) Matrix of fixed node coordinates
%   C   = (m x n) Connectivity matrix (m=b+s, n=q+p)
%   U   = (dim x q) Applied external force vector at each free node
%   extra_constraints = (Optional) Symbolic constraints (e.g., for pulleys)
%
% OUTPUTS:
%   c_bars    = Vector of compression forces in bars (negative value indicates tension)
%   t_strings = Vector of tension forces in strings (negative value indicates compression)
%   V         = (dim x p) Matrix of reaction forces at fixed nodes
...
for i=1:m; ell(i)=norm(M(:,i)); D(:,i)=M(:,i)/ell(i); end % element lengths ell(i), Direction vectors D(:,i)
x=sym('x',[1 m]); X=diag(x);        % set up (3a) in B2019 symbolically
sys=D*X*CQ-U;                       % note that sys=sys_(dim x q). We seek diagonal X subject to sys=0.
...
mhat=dim*q, nhat=m        % note that Ase=Ase_(mhat x nhat) mhat-number of equili equations nhat-number of unknown member forces
[U_,S_,V_]=svd(Ase); s_=diag(S_);                 % compute SVD of Ase
...
if DOF>0
    ...
    % Start with x=0 solution, corresponding to u=0. System is pretentionable if a solution to the LP (5a)-(5b) exists with all strings in tension.
