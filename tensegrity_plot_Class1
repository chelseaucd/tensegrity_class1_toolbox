...
% TENSEGRITY_PLOT Visualization of Class 1 tensegrity structures and forces.
%
% This function generates a 2D or 3D representation of the tensegrity system.
% It is specifically styled to facilitate the verification of CLASS 1 TOPOLOGY,
% allowing the user to visually confirm that compression members (bars) are 
% isolated from one another.
%
% VISUALIZATION KEY:
%   - Bars (Compression):    RED lines. (In Class 1, red lines should not touch).
%   - Strings (Tension):     BLACK lines (3D) or BLUE lines (2D).
%   - External Forces (U):   MAGENTA arrows.
%   - Reaction Forces (V):   BLUE arrows.
%   - Fixed Support Nodes:   Indicated by tetrahedrons (3D) or triangles (2D).
%
% DEPENDENCIES:
%   - Requires 'mArrow3.m' for 3D arrow visualization.
%
% INPUTS:
...
%   b     = Number of bars
%   s     = Number of strings
%   U     = (dim x q) Applied external force vector (optional)
%   V     = (dim x p) Reaction force vector (optional)
%   outer = (bool) If true, arrows point outward from nodes. Default: false.
%   fac   = (scalar) Scaling factor for force arrow length. Default: 1.
%   fac1  = (scalar) Scaling factor for arrow tip size. Default: 1.
%
% USAGE:
%   tensegrity_plot(Q,P,C,b,s)              % Geometry only
%   tensegrity_plot(Q,P,C,b,s,U,V)          % Geometry + Forces
%   tensegrity_plot(Q,P,C,b,s,U,V,true,2,1) % Customized arrow appearance
